...
class AddressSelect extends React.Component {
    constructor(props){
        ...
        this.state={
            ...
            let data = this.props.parame || {}

            api.getAddressAll(data).then(async (obj)=>{
                if (obj.length===0) {
    ...
    goEdit(id,e){
        let url = window.location.pathname + window.location.search;
        location.href='/vpage/addAddress?addressId='+id+"&returnUrl="+encodeURIComponent(url);
    }
    render() {
        let _list=[];
        if(!this.state.list){
                        ...
                        <div className="address_select_list">
                            {
                                _list.map((item,index)=>{
                                    let _support='';
                                    if(!item.support){
                                        _support=<div className="address_select_unsupport"> <img width="105" src="http://resource.sa-green.cn/image/address%E5%8D%B0%E7%AB%A0.png"/></div>;
                                    }
                                    let _flag='';
                                    if(item.addressFlag&&item.addressFlag!=''&&item.addressFlag!='0'){
                                        _flag=<span className="address_select_item_tag">{item.addressFlag}</span>
                                    }
                                    ...
                                    return <div onTouchStart={this.selectAddress.bind(this,index,item.support)} key={index} data-id={item.idStr} className="address_select_item" >
                                        ...
                                        <div className="address_select_item_center">
                                            <div className="address_select_item_info">
                                                <span className="address_select_item_name">{item.contactName}</span>
                                                <span  className="address_select_item_phone">{item.contactNo}</span>
                                                {_flag}
                                            </div>
                                            <div className="address_select_item_address">
                                                <span className="address_select_item_area">{item.regionName}</span> <span className="address_select_item_address_detail">{item.addressDetail}</span>
                                            </div>
                                        </div>
