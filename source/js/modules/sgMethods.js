/**
 * Created by peiyuanwu_sagreen on 2016/12/15.
 * 通用方法对象
 *
 */
...
methods.wxShare=function(share){
  //let _arr=location.href.split('#')[0].split('/');
  //let _url=location.href.replace(_arr[_arr.length-1],'');
  let _url=location.href.split('#')[0];

  api.getWechatConfig({'url':_url})
  .then((res)=>{
    let configInfo={
      debug: false,
      appId: res.appid,
...
methods.wxShareInfo=function(configInfo,share){
  if(!share){
    share={
      title: '沙绿轻食', // 分享标题
      desc: '生活，从此有沙绿', // 分享描述
      link: 'http://' + location.hostname + '/vpage/sgIndex', // 分享链接
      imgUrl: 'http://resource.sa-green.cn/images/logo/sagreenLogo.png', // 分享图标
